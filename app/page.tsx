  ...
  return (
    <main className="brand-shell min-h-screen overflow-hidden">
      ...
      <div className="relative z-10 mx-auto w-full max-w-6xl px-4 pb-12 pt-7 sm:px-6 sm:pb-16 sm:pt-12 lg:px-8">
        ...
        {prediction && <section ref={resultRef} className="result-reveal mx-auto mt-8 max-w-4xl sm:mt-12" aria-live="polite"><div className="celebration-layer" aria-hidden="true"><span>🎉</span><i /><i /><i /><i /><i /><i /><i /><i /><b>🎊</b></div><div className="result-card overflow-hidden rounded-[1.75rem] sm:rounded-[2.25rem]"><div className="result-top p-5 text-white sm:p-8"><div className="flex items-center justify-between gap-4"><div><p className="congrats-line">Congratulations!</p><p className="mt-1 text-[10px] font-bold uppercase tracking-[.25em] text-white/60">Estimated All India Rank</p><h2 className="headline mt-1.5 text-2xl font-black sm:text-4xl">{candidateName}, your AIR could be</h2></div><div className="score-chip"><span>Score</span><strong>{prediction.enteredScore}</strong></div></div></div><div className="bg-white p-5 sm:p-8"><p className="result-rank headline gradient-text text-5xl font-black sm:text-7xl">{formatRank(prediction.predictedRank)}</p><div className="mt-5 grid gap-3 sm:grid-cols-2"><div className="result-stat"><span>Expected range</span><strong>{formatRank(prediction.rankFrom)} – {formatRank(prediction.rankTo)}</strong></div><div className="result-stat"><span>Prediction basis</span><strong>Your score + 2026 trend</strong></div></div><p className="mt-5 text-xs leading-5 text-slate-400">Estimated from your expected score, with last year’s 800-mark trends adjusted to the current 720-mark scale. The official rank may vary.</p></div></div></section>}
